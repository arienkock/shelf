...
[ -z "$JVM_ARGS" ] && \
JVM_ARGS="-server -Xms64m -Xmx512m -XX:+UseG1GC -XX:MaxMetaspaceSize=64m -Dsun.net.inetaddr.ttl=60 -Dsun.net.client.defaultConnectTimeout=5000 -Dsun.net.client.defaultReadTimeout=5000"
...
(cd "$APP_DIR" && mkdir -p bin lib config logs && chown "$APP_USER:$APP_USER" ./ bin/ lib/ config/ logs/)
